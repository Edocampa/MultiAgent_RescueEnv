"""
grid_world.py - Parametric grid environment.

NEW IN v3:
    - Works for ANY grid size N (4, 6, 12, 24, 48, ...)
    - Local view size is configurable per level (e.g. 5x5 for 10x10,
      11x11 for 24x24, etc.). Critical to avoid the "agent is blind" issue
      when scaling up.
    - Observation and global state dimensions scale automatically.
"""
...
def get_view_size_for(grid_size, max_view=11):
    """
    Default view size policy: scale with grid size but cap at max_view.
    For grids smaller than max_view, the view IS the whole grid.
    """
...
def get_obs_dim(size, view_size=None):
    """
    Returns (obs_dim, view_size).
    obs_dim = 10 base features + view_size**2 local view.
    """
...
def get_global_dim(size):
    """Global state dim: 10 base + size*size obstacle map."""
    return 10 + size * size


class SimpleGridWorld:
    """
    Grid environment parameterized by (size, view_size).

    Reset behaviors:
        - reset(concrete_map=None): generates a random map at self.size
        - reset(concrete_map=<dict>): uses the provided map; if its size
          differs from self.size, it's projected automatically.
    """
    ...
    def reset(self, concrete_map=None):
        """
        Reset the env. If concrete_map is provided, use it; otherwise generate.
        """
        if concrete_map is None:
            map_data = generate_concrete_map(
                ...
                random_walls=self.random_walls,
                rng=self._rng,
            )
        else:
        ...
        # Edge case at coarse levels: agent may project onto item
    ...
    def get_observations(self):
        """Returns observations for both agents. Same structure as v2."""
        N = max(1, self.size - 1)
