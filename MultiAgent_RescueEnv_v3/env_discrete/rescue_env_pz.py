"""rescue_env_pz.py - PettingZoo wrapper for discrete env (from v3)."""
...
class RescueEnvPZ(ParallelEnv):
    metadata = {"render_modes": [None], "name": "rescue_v4_discrete"}

    def __init__(self, size, max_cycles=200, num_fires=2, wall_density=0.10,
                 random_walls=True, view_size=None, seed=None):
        self.size = size
        self.max_cycles = max_cycles
        self._step_count = 0
        self._env = SimpleGridWorld(size=size, num_fires=num_fires,
        ...
        self._action_spaces = {a: spaces.Discrete(5)
                                for a in self.possible_agents}
