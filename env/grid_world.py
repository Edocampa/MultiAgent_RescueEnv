"""
Parameterized grid environment

The environment accepts a PRE-GENERATED concrete_map dict at reset time.
This means the SAME physical 10x10 map (projected to the appropriate level
via map_generator.project_map) is used at all three levels of the hierarchy
within a given episode

Dimensions adapt to grid size:
    size=10: obs_dim=35  (10 base + 25 local 5x5 view), global_dim=110
    size=5:  obs_dim=35  (10 base + 25 view; the 5x5 view IS the whole grid)
    size=3:  obs_dim=19  (10 base + 9 view; 3x3 view IS the whole grid)
"""
...
from map_generator import (
    generate_concrete_map, project_map,
    phi_10_to_5, phi_5_to_3, phi_10_to_3,
)
...
def get_obs_dim(size):
    """Returns (obs_dim, view_size) for a given grid size"""
    view_size = min(5, size)
    return 10 + view_size * view_size, view_size


def get_global_dim(size):
    """Returns global state dim: 10 base + size*size obstacle map"""
    return 10 + size * size


class SimpleGridWorld:

    def __init__(self, size=10, num_fires=2,random_walls=False, num_random_walls=8, seed=None):
        self.size = size
        ...
        self.obs_dim, self.view_size = get_obs_dim(size)
    ...
    def reset(self, concrete_map=None):
        ...
        if concrete_map is None:
            if self.size != 10:
                raise ValueError(
                    f"Cannot generate a {self.size}x{self.size} map directly. "
                    f"Pass concrete_map=<10x10 dict> instead."
                )
            map_data = generate_concrete_map(
                num_fires=self.num_fires, 
                random_walls=self.random_walls,      # Passa il flag
                num_random_walls=self.num_random_walls, # Passa il numero di muri 
                rng=self._rng
            )
        else:
            # Project the concrete map to this level if needed
            if concrete_map["size"] != 10:
                raise ValueError("concrete_map must be size 10")
            if self.size == 10:
        ...
        # Edge case: at very coarse levels (3x3), agents might project to the
        # same cell. Allow this but warn, they can move apart in step 1
        # Edge case: agent might project onto item - handle this in step
        ...
        # Edge case: at coarse levels, agent might start on item
        # This counts as immediate pickup
