"""
Multi-Agent PPO with optional dual learner

dual_learner=True implements:
    - biased actors:   trained on (base_reward + shaping)
    - unbiased actors: trained on (base_reward only)
    Both share the same trajectory; their gradient updates differ.
    Actions are sampled from the biased actors (faster exploration)
    The unbiased actors are the final policy

dual_learner=False: single learner (used for sparse experiments where
    there's no shaping anyway)
"""
...
class MAPPO:
    def __init__(
        ...
        self.actors_biased = [
            Actor(obs_dim, action_dim, hidden_dim).to(device)
    ...
    def select_actions(self, obs_list, global_state):
        """Selects actions from BIASED actors. Returns values from both critics if dual."""
        actions = []
        log_probs = []
    ...
    def update(self):
        m = {"biased": self._update_one(
            ...
            self.actor_opts_biased, self.critic_biased, self.critic_opt_biased
        )}
        if self.dual_learner:
            m["unbiased"] = self._update_one(
                self.buffer_unbiased, self.actors_unbiased,
                self.actor_opts_unbiased, self.critic_unbiased, self.critic_opt_unbiased
            )
        return m
